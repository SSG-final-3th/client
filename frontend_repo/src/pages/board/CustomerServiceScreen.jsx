...
export async function deleteQuestion(questionId, token) {
  try {
    const response = await axios.delete(
      `http://k9testspringboot-env.eba-kduvbera.us-east-2.elasticbeanstalk.com/emart/questions/delete/${questionId}`,
      {
        headers: { Authorization: `Bearer ${token}` },
      }
    );
  ...
  const fetchInquiries = async () => {
    ...
    try {
      const response = await axios.get(
        `http://k9testspringboot-env.eba-kduvbera.us-east-2.elasticbeanstalk.com/emart/questions/${userId}`,
        {
          headers: {
